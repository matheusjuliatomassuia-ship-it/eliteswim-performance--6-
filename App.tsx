...
const App: React.FC = () => {
  ...
  const [metrics, setMetrics] = useState<DailyMetric[]>(() => {
    const saved = localStorage.getItem('eliteSwim_metrics');
    return saved ? JSON.parse(saved) : MOCK_METRICS;
  });

  const [times, setTimes] = useState<SwimTime[]>(() => {
    const saved = localStorage.getItem('eliteSwim_times');
  ...
  const [pbRecords, setPbRecords] = useState<Record<string, Record<string, Record<string, string>>>>(() => {
    const saved = localStorage.getItem('eliteSwim_pbRecords');
    return saved ? JSON.parse(saved) : {};
  });

  const [meetRecords, setMeetRecords] = useState<MeetRecord[]>(() => {
    const saved = localStorage.getItem('eliteSwim_meetRecords');
    return saved ? JSON.parse(saved) : [];
  });

  const [resources, setResources] = useState<Resource[]>(() => {
    const saved = localStorage.getItem('eliteSwim_resources');
    return saved ? JSON.parse(saved) : MOCK_RESOURCES;
  });

  const [bodyData, setBodyData] = useState<BodyComposition[]>(() => {
    const saved = localStorage.getItem('eliteSwim_body');
    return saved ? JSON.parse(saved) : MOCK_BODY;
  });

  const [gymPlans, setGymPlans] = useState<WorkoutPlan[]>(() => {
    const saved = localStorage.getItem('eliteSwim_gymPlans');
    return saved ? JSON.parse(saved) : [];
  });

  const [gymLogs, setGymLogs] = useState<GymLog[]>(() => {
    const saved = localStorage.getItem('eliteSwim_gymLogs');
    return saved ? JSON.parse(saved) : [];
  });

  const [competitions, setCompetitions] = useState<Competition[]>(() => {
    const saved = localStorage.getItem('eliteSwim_competitions');
    return saved ? JSON.parse(saved) : MOCK_COMPETITIONS;
  });

  const [strategies, setStrategies] = useState<Strategy[]>(() => {
    const saved = localStorage.getItem('eliteSwim_strategies');
    return saved ? JSON.parse(saved) : MOCK_STRATEGIES;
  });

  const [assessments, setAssessments] = useState<PhysicalAssessment[]>(() => {
    const saved = localStorage.getItem('eliteSwim_assessments');
    return saved ? JSON.parse(saved) : MOCK_ASSESSMENTS;
  });

  const [biomechanics, setBiomechanics] = useState<BiomechanicsAnalysis[]>(() => {
    const saved = localStorage.getItem('eliteSwim_biomechanics');
    return saved ? JSON.parse(saved) : MOCK_BIOMECHANICS;
  });

  const [plannedSessions, setPlannedSessions] = useState<PlannedSession[]>(() => {
    const saved = localStorage.getItem('eliteSwim_plans');
    return saved ? JSON.parse(saved) : MOCK_PLANS;
  });

  const [managedAthletes, setManagedAthletes] = useState<AthleteProfile[]>(() => {
    const saved = localStorage.getItem('eliteSwim_managedAthletes');
    return saved ? JSON.parse(saved) : MOCK_ATHLETE_PROFILES;
  });

  const [managedGroups, setManagedGroups] = useState<Group[]>(() => {
    const saved = localStorage.getItem('eliteSwim_managedGroups');
    return saved ? JSON.parse(saved) : MOCK_GROUPS;
  });

  const [managedWorkouts, setManagedWorkouts] = useState<WorkoutDefinition[]>(() => {
    const saved = localStorage.getItem('eliteSwim_managedWorkouts');
    return saved ? JSON.parse(saved) : MOCK_WORKOUT_TYPES;
  });

  useEffect(() => { localStorage.setItem('eliteSwim_metrics', JSON.stringify(metrics)); }, [metrics]);
  useEffect(() => { localStorage.setItem('eliteSwim_times', JSON.stringify(times)); }, [times]);
  useEffect(() => { localStorage.setItem('eliteSwim_pbRecords', JSON.stringify(pbRecords)); }, [pbRecords]);
  useEffect(() => { localStorage.setItem('eliteSwim_meetRecords', JSON.stringify(meetRecords)); }, [meetRecords]);
  useEffect(() => { localStorage.setItem('eliteSwim_plans', JSON.stringify(plannedSessions)); }, [plannedSessions]);
  useEffect(() => { localStorage.setItem('eliteSwim_gymPlans', JSON.stringify(gymPlans)); }, [gymPlans]);
  useEffect(() => { localStorage.setItem('eliteSwim_gymLogs', JSON.stringify(gymLogs)); }, [gymLogs]);
  useEffect(() => { localStorage.setItem('eliteSwim_managedAthletes', JSON.stringify(managedAthletes)); }, [managedAthletes]);
  ...
  const handleUpdateUser = (name: string, email: string, photoUrl?: string, coachEmailInput?: string, club?: string, category?: string, birthDate?: string, gender?: 'M' | 'F', state?: string, country?: string, newPassword?: string) => {
    ...
    // Atualiza banco de contas local
    ...
    // Lógica de Sincronização de Atleta (Técnico vê dados)
    if (userRole === 'Athlete') {
  ...
  const handleLogin = (email: string, role: UserRole, name?: string, password?: string) => {
    ...
    // Busca no banco de contas local se existir
    const accountsRaw = localStorage.getItem('eliteSwim_accounts');
    const accounts = accountsRaw ? JSON.parse(accountsRaw) : [];
    const existingAccount = accounts.find((a: any) => a.email.toLowerCase() === email.toLowerCase());

    // Padronização de IDs para consistência entre atleta e coach
    ...
    const newUser = existingAccount ? { ...existingAccount, password: password || existingAccount.password } : { 
      id: finalId, 
      email, 
      password: password || '',
      name: finalName, 
    ...
    // Salva Sessão e Banco de Contas
            ...
            <div className="p-4 border-t border-white/5 bg-black/20">
              <div className="flex items-center gap-3 p-2">
                <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/10">
                  {userPhoto ? <img src={userPhoto} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full bg-slate-800 flex items-center justify-center text-primary font-black italic">{userName.charAt(0)}</div>}
                </div>
            ...
            {isMobileMenuOpen && (
              <div className="lg:hidden fixed inset-0 z-[60] bg-slate-950/95 backdrop-blur-md pt-20 px-6 animate-in slide-in-from-top-full duration-300">
                <div className="grid grid-cols-1 gap-1 overflow-y-auto max-h-[80vh]">
                  ...
                  <button onClick={handleLogout} className="flex items-center p-4 rounded-2xl text-xs font-black uppercase italic text-danger bg-danger/10 mt-4"><LogOut size={20} className="mr-4" /> Encerrar Sessão</button>
                </div>
              </div>
            )}
